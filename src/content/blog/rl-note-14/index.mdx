...
## 2. 基于能量的模型 (Energy-Based Model, EBM)
...
## 3. Soft 贝尔曼方程 (Soft Bellman Equation)
...
**Soft 状态价值函数 $V_{soft}$ 推导：**
$V_{soft}$ 不仅包含未来的奖励，还包含未来的熵。
...
## 4. Soft 策略提升定理 (Soft Policy Improvement)
...
$$
\begin{align}
\tilde{\pi}(\cdot|s) \propto \exp(\frac{1}{\alpha}Q^{\pi}_{\textbf{soft}}(s,\cdot)), \quad \forall s \notag \\
\end{align}
$$
...
**关键推导步骤**（此处更正了原笔记中关于 $\alpha$ 的处理细节）：
...
2.  积分保序性：
    $$
    \int \exp(\frac{Q_1}{\alpha}) da' \le \exp(\frac{\epsilon}{\alpha}) \int \exp(\frac{Q_2}{\alpha}) da'
    $$

3.  取 $\alpha \log$（**注意这里的常数提取**）：
    $$
    \begin{align}
    \alpha \log \int \exp(\frac{Q_1}{\alpha}) da' &\le \alpha \log \left[ \exp(\frac{\epsilon}{\alpha}) \int \exp(\frac{Q_2}{\alpha}) da' \right] \notag \\
    &= \alpha \left[ \log(\exp(\frac{\epsilon}{\alpha})) + \log \int \exp(\frac{Q_2}{\alpha}) da' \right] \notag \\
    &= \alpha \left[ \frac{\epsilon}{\alpha} + \log \int \exp(\frac{Q_2}{\alpha}) da' \right] \notag \\
    &= \epsilon + \alpha \log \int \exp(\frac{Q_2}{\alpha}) da' \notag
    \end{align}
    $$
    > **💡 修正说明**：
    > 注意这里 $\alpha \times \frac{\epsilon}{\alpha} = \epsilon$。原笔记推导得到的系数是 $\alpha\epsilon$，但正确数学推导后系数应为 $1\cdot\epsilon$。这很重要，因为这意味着收敛性不依赖于 $\alpha$ 的大小。
