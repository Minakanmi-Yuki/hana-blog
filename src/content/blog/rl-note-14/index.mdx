...
description: '深入解析 Soft Q-Learning (SQL)：从最大熵强化学习原理出发，详细推导 Soft Bellman 方程、策略提升定理及收敛性证明，并探讨基于能量模型的策略采样与实现细节。'
heroImage: { src: 'https://pic.hana0721.top/86800103_p0_master1200.4qrwyrtj6a.webp', color: '#8C8275' }
tags:
  - '强化学习'
  - 'RL笔记'
  - 'SQL'
...
4.  计算算子差值：
    ...
    $$
    \begin{align}
    |\mathcal{T}Q_1 - \mathcal{T}Q_2| &= \gamma \left| \mathbb{E}_{s'} \left[ \alpha \log \int_{a^\prime} \exp(\frac{Q_1(s^\prime,a^\prime)}{\alpha}) \text{d}a^\prime - \alpha \log \int_{a^\prime} \exp(\frac{Q_2(s^\prime,a^\prime)}{\alpha}) \text{d}a^\prime \right] \right| \notag \\
...
由于折扣因子 $\gamma \in (0,1)$，算子 $\mathcal{T}$ 是一个**压缩映射**，必然收敛到唯一的不动点 $Q^*_\text{soft}$。
...
在计算 $V_\text{soft}(s)$ 时，需要计算 $\log \int \exp(Q) \text{d}a$。
