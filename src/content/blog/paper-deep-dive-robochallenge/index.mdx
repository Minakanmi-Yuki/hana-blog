...
![RoboChallenge 远端机器人接口与异步观测、动作队列（论文 Figure 1）](https://raw.githubusercontent.com/Minakanmi-Yuki/picx-images-hosting/master/blog/paper-deep-dive-robochallenge/remote-robot-api-crop.45iemgxa5j.webp)
...
![Visual Task Reproduction：测试员以参考画面叠加实时相机来复现任务初始状态（论文 Figure 5）](https://raw.githubusercontent.com/Minakanmi-Yuki/picx-images-hosting/master/blog/paper-deep-dive-robochallenge/visual-task-reproduction-crop.362b9auj2m.webp)
...
<Algorithm number={1} title='参与者的训练与评测准备' inputLabel='输入' outputLabel='输出'>
  <div slot='input'>任务 demonstrations、参考 episode 留出集、候选 VLA / policy、RoboChallenge API 客户端</div>
  ...
  <AlgorithmStep>由参与者自行 fine-tune policy，并将其 observation / action 格式适配为平台 API</AlgorithmStep>
...
<Algorithm number={2} title='远端机器人评测执行循环' inputLabel='输入' outputLabel='输出'>
  ...
  <AlgorithmStep>轮询 job；获分配后取得 robot ID、校准时钟并在 `ready` 状态启动机器人</AlgorithmStep>
  <AlgorithmStep keyword='while'>job 状态为 `running`</AlgorithmStep>
  <AlgorithmStep indent={1}>请求带时间戳的 state；若机器人异常或 pending queue 非空，则等待</AlgorithmStep>
  <AlgorithmStep indent={1}>用户本地执行 $A_t=\pi(o_t)$，保持其自己的后处理、同步与 action chunk 策略</AlgorithmStep>
  <AlgorithmStep indent={1}>将 $A_t$ 与 duration 提交到 FIFO queue；平台依序执行并录制</AlgorithmStep>
...
![Table30 的算法挑战、机器人类型、场景位置和目标物体分布（论文 Figure 7）](https://raw.githubusercontent.com/Minakanmi-Yuki/picx-images-hosting/master/blog/paper-deep-dive-robochallenge/table30-task-distribution-crop.4n8gb1ynvm.webp)
...
![Table30 各模型的 Success Rate 与 Progress Score 累积分布（论文 Figure 8）](https://raw.githubusercontent.com/Minakanmi-Yuki/picx-images-hosting/master/blog/paper-deep-dive-robochallenge/table30-score-distribution-crop.4qs28rrqnc.webp)
