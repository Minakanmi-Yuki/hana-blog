...
$$
\pi^\prime(s) = \argmax_{a\in\mathcal{A}} Q^\pi(s,a) = \argmax_{a\in\mathcal{A}} \left( r(s,a)+\gamma\sum_{s^\prime \in\mathcal{S}}P(s^\prime|s,a)V^\pi(s^\prime) \right)
$$
定理保证：
...
3.  策略提升：
    ...
    *   For each $s \in \mathcal{S}$:
        ...
        *   $\pi(s) \leftarrow \argmax_a \sum_{s\prime, r} P(s\prime,r|s,a)[r + \gamma V(s\prime)]$
