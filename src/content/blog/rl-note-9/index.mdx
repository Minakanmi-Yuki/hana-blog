...
heroImage: { src: 'https://pic.hana0721.top/74502138_p0_master1200.7i0z6ufn7r.webp', color: '#8C8275' }
tags:
  - '强化学习'
  - 'RL笔记'
  - 'REINFORCE'
...
- **离散动作**（如 Softmax）：
  $$ \pi(a|s;\theta)=\frac{\exp(Q*\theta(s,a))}{\sum*{a^\prime}\exp(Q\_\theta(s,a^\prime))} $$
- **连续动作**（如高斯分布）：
  $$ \pi(a|s;\theta) = \frac{1}{\sqrt{2\pi}\sigma} \exp\left(-\frac{(a-\mu*\theta(s))^2}{2\sigma^2}\right) $$
  通常网络输出均值 $\mu*\theta(s)$ 和标准差 $\sigma_\theta(s)$。
...
- **连续动作**：$$ a=\mu\_\theta(s) $$ （可微，用于 DDPG 等算法）
...
利用状态访问分布 $\nu^{\pi_\theta}(s)$ 的定义：
$$ \sum*{k=0}^\infty \gamma^k d^{\pi*\theta}(s*0 \to s, k) \propto \nu^{\pi*\theta}(s) $$
...
$$ Q^{\pi*\theta}(s_t, a_t) \approx G_t = \sum*{k=t}^T \gamma^{k-t} r_k $$
