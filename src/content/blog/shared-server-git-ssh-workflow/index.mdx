...
category: 'tech'
heroImage: { src: 'https://Minakanmi-Yuki.github.io/picx-images-hosting/101003773_p0_master1200.3uvfruqg2c.webp' }
tags:
  - 'git'
  - 'github'
  - 'ssh'
...
1. 右上角头像 -> `Settings`
2. 左侧进入 `SSH and GPG keys`
3. 点击 `New SSH key`
4. `Key type` 选择 **Authentication Key**
5. `Title` 填一个容易识别的名称（例如 `my-laptop-agent-forward`）
6. `Key` 粘贴整行公钥
7. 点击 `Add SSH key`
...
1. 确认你已经完成上面的 `Settings` -> `SSH and GPG keys` -> `New SSH key`
2. 检查粘贴的是完整公钥（包含开头 `ssh-ed25519` 和中间长串内容）
3. 保存后重新测试：
...
1. 到 GitHub `Settings` -> `Emails` 确认邮箱已验证，或复制你的 `noreply` 邮箱
2. 在仓库重新设置：
