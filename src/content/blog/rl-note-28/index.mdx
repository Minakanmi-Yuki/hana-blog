...
heroImage: { src: 'https://pic.hana0721.top/139369087_p0_master1200.9gx5x7w6ei.webp', color: '#8C8275' }
tags:
  - '强化学习'
  - 'RL笔记'
  - 'RLHF'
...
3.  训练一个标量奖励模型 $r_\theta(x, y)$，使其输出的分数符合人类的排序规律。损失函数通常采用 **Pairwise Ranking Loss**：
    $$ L(\theta) = - \mathbb{E}_{(x, y_w, y_l) \sim \mathcal{D}} \left[ \log \sigma \left( r_\theta(x, y*w) - r*\theta(x, y_l) \right) \right] $$
