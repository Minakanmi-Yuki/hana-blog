...
为了训练模型理解「语义取向」，SOFAR 构建了一个大规模的「语义-几何」对齐数据集，名为 OrienText300K。该数据集基于 VLM 与 Objaverse 生成，包含了 350K 个物体配对的语义取向标签。OrienText300K 的数据由三元组 [3D 网络模型, 语言描述, 语义取向]，其中语义取向表示为语言描述对应的单位方向向量。
