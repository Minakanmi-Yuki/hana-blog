...
$$
其中 $Q'_{tot}(s, \mathbf{u}) = \sum Q_i(u_i)$ 是我们实际用来选动作的函数。
...
$$
L_{opt} = (Q'_{tot}(\bar{\mathbf{u}}) - y_{target})^2 + \lambda \sum_{\mathbf{u} \ne \bar{\mathbf{u}}} (\dots)^2
$$
...
### 损失函数
$$
\mathcal{L} = \sum w(s, \mathbf{u}) \left( \hat{Q}^*(s, \mathbf{u}) - Q_{tot}(s, \mathbf{u}) \right)^2
$$
权重函数 $w$ 的设计体现了**乐观主义**：
