...
heroImage: { src: 'https://pic.hana0721.top/105886628_p0_master1200.5fl6its319.webp', color: '#8C8275' }
tags:
  - '强化学习'
  - 'RL笔记'
  - 'QTRAN'
...
1.  **最优动作一致性**：在最优动作 $\bar{\mathbf{u}}$ 处，两者相等。
    $$ Q'_{tot}(\bar{\mathbf{u}}) - Q_{tot}(\bar{\mathbf{u}}) + V\_{tot}(\bar{\mathbf{u}}) = 0 $$
2.  **非最优动作界限**：在非最优动作 $\mathbf{u}$ 处，$Q'_{tot}$ 不会“篡位”。
    $$ Q'_{tot}(\mathbf{u}) - Q_{tot}(\mathbf{u}) + V\_{tot}(\mathbf{u}) \ge 0 $$
...
$$ Q*{tot}(s, \mathbf{u}) = V*{tot}(s) + A\_{tot}(s, \mathbf{u}) $$
