...
**【Code】** code_gen/task_generation_mm.py 会写出 envs_gen/gpt_<task>.py，调用 insert_observation_points 注入观测点，再由 run() 汇总 success_rate、error_message 和 run_records。仓库默认每个候选程序最多尝试 5 次，test_gen_code.py 中的模拟评估以 10 次 rollout 形成成功率。
