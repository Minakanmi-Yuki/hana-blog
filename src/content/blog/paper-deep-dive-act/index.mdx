...
```text
Training:
[CLS] + joints + action sequence
  -> CVAE encoder (transformer encoder)
  -> mu, logvar -> sample z (32 dim)

4 RGB images -> ResNet18 -> image tokens
qpos (14) -> linear -> 512
z (32) -> linear -> 512
concat -> transformer encoder -> memory
  -> transformer decoder (k queries)
  -> action head -> k x 14

Inference:
same observations -> z = 0 -> predict k x 14
```
...
- 推理输入：当前 4 路 RGB 图像和当前关节位置。
- 推理过程：CVAE encoder 被丢弃，$z$ 固定为 prior 均值，即零向量。
- Action 产生方式：transformer decoder 一次性输出未来 `k` 步目标关节位置。
- Action 转换：目标关节位置交给 Dynamixel 的低层 PID 控制器跟踪；论文强调使用 leader 机器人关节位置而不是 follower 位置，因为两者差值隐含力的大小。
- Action Horizon / Chunking：默认 `k = 100`；不使用 temporal ensemble 时每隔 `k` 步重新查询，使用 temporal ensemble 时每个时间步都查询。
