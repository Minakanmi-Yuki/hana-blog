...
heroImage: { src: 'https://pic.hana0721.top/112236596_p0_master1200.232gogbiox.webp', color: '#8C8275' }
tags:
  - '强化学习'
  - 'RL笔记'
  - 'MADDPG'
...
1.  **Critic 更新**：最小化贝尔曼误差。
    $$ y = r*i + \gamma Q_i'(s', a'\_1, \dots, a'\_N)|*{a'\_j = \mu'\_j(o'\_j)} $$
    注意：计算目标值时，需要用到**每个智能体的 Target Actor** 来预测下一步动作。
2.  **Actor 更新**：确定性策略梯度。
    $$ \nabla*{\theta_i} J = \mathbb{E} [\nabla*{a*i} Q_i(s, a_1, \dots, a_N)|*{a*i=\mu_i(o_i)} \cdot \nabla*{\theta_i} \mu_i(o_i)] $$
...
$$ \hat{A}_i(t) = \sum (\gamma \lambda)^l (r_{i, t+l} + \gamma V(s*{t+1+l}) - V(s*{t+l})) $$。
