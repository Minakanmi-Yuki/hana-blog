...
![DexUMI 从人类示教到不同机器人手执行的总览（论文 Teaser）](https://raw.githubusercontent.com/Minakanmi-Yuki/picx-images-hosting/master/blog/paper-deep-dive-dexumi/teaser.webp)
...
![DexUMI 的硬件适配架构（论文 Figure 2）](https://raw.githubusercontent.com/Minakanmi-Yuki/picx-images-hosting/master/blog/paper-deep-dive-dexumi/hardware-method.webp)
...
![外骨骼与传感器设计（论文 Figure 2）](https://raw.githubusercontent.com/Minakanmi-Yuki/picx-images-hosting/master/blog/paper-deep-dive-dexumi/hardware-method.webp)
...
![视觉适配流水线：分割、背景修复与机器人手合成（论文 Figure 3）](https://raw.githubusercontent.com/Minakanmi-Yuki/picx-images-hosting/master/blog/paper-deep-dive-dexumi/software-method.webp)
...
$$
\max_{\mathbf p}\;\mathcal S(\mathcal W_{\mathrm{exo}}^{\mathrm{tip}}(\mathbf p),\n+\mathcal W_{\mathrm{robot}}^{\mathrm{tip}})
$$

其中 (mathcal W^{\mathrm{tip}}) 是指尖位姿集合，(mathbf p) 是外骨骼参数。实现时两侧 workspace 采样并计算最近邻距离；一项鼓励覆盖机器人空间，另一项抑制不可达姿态。**【Analysis】** 这可看作带边界约束的对称 Chamfer-like workspace 距离。
...
其中 (a_s) 为第 (s) 步的 noisy action，(o_t) 为视觉与本体输入，(f_t) 为触觉。**【Code】** `train_diffusion_policy.py` 明确生成 `timesteps`、`noisy_actions` 并调用模型计算 loss。
...
![Cube、Egg Carton、Tea 与 Kitchen 四类任务（论文 Figure 4）](https://raw.githubusercontent.com/Minakanmi-Yuki/picx-images-hosting/master/blog/paper-deep-dive-dexumi/experiment.webp)

**【Paper】** 每任务评估 20 个 episode，并复用各方法的初始物体配置。Cube 测试精确抓取；Egg Carton 测试多指协同；Tea 要求用镊子夹茶叶；Kitchen 包含关火旋钮、搬锅、抓盐和撒盐。

### 5.2 Main Results

![相对/绝对手动作与触觉对比（论文 Figure 5）](https://raw.githubusercontent.com/Minakanmi-Yuki/picx-images-hosting/master/blog/paper-deep-dive-dexumi/comparison.webp)
...
![DexUMI 的数据采集效率对比（论文 Figure 6）](https://raw.githubusercontent.com/Minakanmi-Yuki/picx-images-hosting/master/blog/paper-deep-dive-dexumi/efficiency.webp)
